...
# Quitamos la columna de CustomerID para que no interfiera en los datos
...
#con coeficiente de silhouette de 0.62 
#Como por el metodo del codo tenemos k=4, sabemos que existen dos grupos 
#cohesionados con dos subgrupos con coeficiente de silhouette de 0.55
...
# aglomerativo es k = 2 con coeficiente de silhouette de 0.61

# Tiene mejor coeficiente de silhouette el k-means

#6. Análisis Descriptivo
#Asignamos al dataframe nuestros modelos
df$cluster_kmeans <- as.factor(km_final$cluster)
df$cluster_jerarquico <- as.factor(clusters_jer)

df_kmeans <- aggregate(df[, c("Genre","Age", "Annual_income", "Spending_score")],
          by = list(Cluster = df$cluster_kmeans),
          FUN = mean)

#Con estos datos podemos ver con el K-MEANS 4 clusteres bien diferenciados:
# Cluster 1: Adultos de 37 con ingresos ligeramente por debajo de la media 
# Cluster 2: Adultos de 35 con ingresos ligeramente por encima de la media 
# Cluster 3: Jovenes de 22 con ingresos ligeramente por debajo de la media 
# Cluster 4: Mayores de 63 con ingresos ligeramente por debajo de la media 

df_jerarquico <- aggregate(df[, c("Genre","Age", "Annual_income", "Spending_score")],
          by = list(Cluster = df$cluster_jerarquico),
          FUN = mean)

#Con estos datos podemos ver con el jeraquico 2 clusteres bien diferenciados:
# Cluster 1: Jovenes de 28 con ingresos por encima de la media
# Cluster 2: Mayores de 51 con un ingreso anual por debajo de la media
